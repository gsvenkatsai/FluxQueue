...
function StatCard({ label, value, accent }: { label: string; value: string | number; accent?: string }) {
  return (
    <div className={`bg-gray-800 rounded-xl p-5 border ${accent ?? "border-gray-700"} shadow-lg`}>
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard label="Pending" value={stats.pending_count} accent="border-yellow-500/40" />
        <StatCard label="Completed" value={stats.completed_count} accent="border-green-500/40" />
        <StatCard
          label="Failure Rate"
        ...
        <StatCard label="Running" value={stats.running_count} accent="border-blue-500/40" />
        <StatCard label="Failed" value={stats.failed_count} accent="border-red-500/40" />
        <StatCard label="Dead" value={stats.dead_count} accent="border-gray-500/40" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-gray-800 rounded-xl p-5 border border-gray-700 shadow-lg md:col-span-1">
          <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-4">Job Status</p>
          <JobStatusPieChart {...stats} />
        </div>
        <div className="bg-gray-800 rounded-xl p-5 border border-gray-700 shadow-lg">
          <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-4">Workers</p>
          <WorkerHealthTable workers={stats.workers} />
        </div>
        <div className="bg-gray-800 rounded-xl p-5 border border-gray-700 shadow-lg">
          <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-4">Avg Exec Time</p>
      ...
      {/* Throughput Chart */}
      <div className="bg-gray-800 rounded-xl p-5 border border-gray-700 shadow-lg">
        <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-4">Throughput — last 60 min</p>
